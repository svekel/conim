...
proc getDownloadInfo*(pv: PkgTuple, options: Options,
                      doPrompt: bool): (bool, DownloadMethod, string,
                                        Table[string, string]) =
  if pv.name.isURL:
    let (url, metadata) = getUrlData(pv.name)
    return (true, checkUrlType(url), url, metadata)
  else:
    var pkg: Package
    if getPackage(pv.name, options, pkg):
      let (url, metadata) = getUrlData(pkg.url)
      return (true, pkg.downloadMethod.getDownloadMethod(), url, metadata)
    else:
      ...
      #                    "check internet for updated packages?"):
        ...
        return (false, DownloadMethod.git, pv.name, initTable[string, string]())
...
proc install(packages: seq[PkgTuple],
  ...
  if packages == @[]:
    ...
    for pv in packages:
      let (success, meth, url, metadata) = getDownloadInfo(pv, options, doPrompt)
      var 
        downloadDir: string
      ...
      if success:
        (downloadDir, downloadVersion) = downloadPkg(url, pv.ver, meth, subdir, options)
      else:
        (downloadDir, downloadVersion) = customDownloadPkg(url, pv.ver, meth, subdir, options)
...
proc develop(options: Options) =
  if options.action.packages == @[]:
    ...
    for pv in options.action.packages:
      ...
      let (_, meth, url, metadata) = getDownloadInfo(pv, options, true)
